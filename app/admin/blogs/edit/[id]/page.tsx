...
import { api, ApiError, Product, formatBase64Image } from "@/lib/api";
    ...
    useEffect(() => {
        ...
        const loadInitialData = async () => {
            try {
                ...
                const [blogData, allProducts] = await Promise.all([
                ...
                if (blogData) {
                    setFormData({
                        ...
                        relatedProductIds: (blogData.relatedProducts || []).map(p => p.id!)
                    });
                    if (blogData.image) {
                        setImagePreview(formatBase64Image(blogData.image));
                    }
                }

                if (Array.isArray(allProducts)) setProducts(allProducts);
            } catch (e) {
                console.error(e);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const blogJson = {
                ...
                relatedProducts: formData.relatedProductIds.map(pid => ({ id: pid } as Product))
            };
                ...
                <div className="max-w-4xl mx-auto px-6 py-6 flex justify-between items-center">
                    <div className="flex items-center gap-4">
                        ...
                        <h1 className="text-2xl font-black tracking-tighter font-heading">
                            Edit <span className="text-[#38A36D]">Blog Post</span>
                        </h1>
                    </div>
                </div>
            </header>
