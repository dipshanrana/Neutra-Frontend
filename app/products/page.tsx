...
function ShopContent() {
    ...
    const handleSearch = async (e: React.FormEvent) => {
        ...
        try {
            let results: Product[] = [];
            if (searchQuery) results = await api.products.searchByName(searchQuery);
            else if (selectedCategory) results = await api.products.searchByCategory(selectedCategory);
            else if (minPrice || maxPrice) {
                const min = minPrice ? parseFloat(minPrice) : 0;
                const max = maxPrice ? parseFloat(maxPrice) : 999999;
                results = await api.products.searchByPriceRange(min, max);
            } else results = await api.products.getAll();
            setProducts(results);
        } catch (e) { console.error(e); }
        finally { setLoading(false); }
    };
