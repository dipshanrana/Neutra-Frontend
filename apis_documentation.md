...
**Request Parts:** Same structure as **Create Product (4.1)**. Include the `product` JSON and any new `MultipartFile` images to update.
*   **Size**: We've increased server limits (10MB per file) so high-res images are supported.
If an image part is not included, the existing image will be kept.
...
## 10. Frontend Image Helpers (`lib/api.ts`)

To ensure consistent image display regardless of which field is populated, use these provided helpers:

### 10.1 `getProductMainImage(product)`
Returns a single source string.
**Priority:** `featuredImages[0]` → `singleProductImage` → `images[0]` → `/multi-vit.png`.

### 10.2 `getProductAllImages(product)`
Returns a unique list of all available source strings for use in galleries.

### 10.3 `formatBase64Image(base64)`
Ensures any raw base64 string from the backend has the correct `data:image/png;base64,` prefix.
