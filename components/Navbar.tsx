        ...
        <div className="flex justify-between items-center h-20">
          ...
          <div className="hidden md:flex items-center space-x-6 pl-4">
            ...
            <div className="relative group/lang">
              <button className="flex items-center gap-1.5 px-3 py-1.5 bg-emerald-50/20 border border-[#252422]/5 rounded-full hover:bg-white hover:border-brand-primary/30 transition-all text-xs font-semibold text-[#252422]/70 group-hover/lang:text-brand-primary">
          ...
          <div className="md:hidden flex items-center gap-4">
            <button
              ...
              className="p-2 text-[#252422] hover:bg-emerald-50 rounded-lg transition-colors"
            >
              {mobileMenuOpen ? (
        ...
        <div className="px-6 py-8 flex flex-col gap-8 pb-32">
          ...
          <form onSubmit={handleSearch} className="relative group">
            <input
              ...
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full pl-5 pr-12 py-4 rounded-2xl border border-[#252422]/10 bg-stone-50 text-[#252422] focus:outline-none focus:border-brand-primary/40 focus:bg-white transition-all shadow-sm"
            />
            <button type="submit" className="absolute right-5 top-1/2 -translate-y-1/2 text-brand-primary">
          ...
          <div className="grid gap-2">
            {[
              ...
              <Link
                key={link.name}
                href={link.href}
                className="text-2xl font-heading font-medium text-[#252422] hover:text-brand-primary py-3 border-b border-[#252422]/5 flex justify-between items-center group"
              >
                {link.name}
            ...
            <div className="grid grid-cols-2 gap-3">
              {categories.map((cat) => (
                <Link
                  key={cat.id}
                  href={`/products?category=${encodeURIComponent(cat.name)}`}
                  className="flex flex-col gap-3 p-4 bg-stone-50 rounded-2xl border border-[#252422]/5 hover:bg-emerald-50 hover:border-emerald-100 transition-all"
                >
                  <div className="w-10 h-10 rounded-xl bg-white shadow-sm flex items-center justify-center text-brand-primary" dangerouslySetInnerHTML={{ __html: cat.svg }} />
          ...
          <div className="mt-4 bg-emerald-50/50 rounded-3xl p-6 border border-emerald-100/50">
